...
class DavidWeekleyHomesMyrtleCreekPlanScraper(BaseScraper):
    ...
    def fetch_plans(self) -> List[Dict]:
        try:
            print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Fetching URL: {self.URL}")
            ...
            print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Response status: {resp.status_code}")
            
            if resp.status_code != 200:
                print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Request failed with status {resp.status_code}")
                return []
            ...
            if not floor_plans:
                print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Floor plans section not found")
                return []
            
            # Find all plan cards in the floor plans section
            plan_cards = floor_plans.find_all('div', class_='plan-card')
            print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Found {len(plan_cards)} plan cards")
            
            for idx, card in enumerate(plan_cards):
                try:
                    print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Processing card {idx+1}")
                    
                    # Extract plan title
                    plan_title = card.find('h2', class_='plan-title')
                    if not plan_title:
                        print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Skipping card {idx+1}: No plan title found")
                        continue
                    
                    plan_name = plan_title.get_text(strip=True)
                    if not plan_name:
                        print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Skipping card {idx+1}: Empty plan name")
                        continue
                    
                    # Check for duplicate plan names
                    if plan_name in seen_plans:
                        print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Skipping card {idx+1}: Duplicate plan name '{plan_name}'")
                        continue
                    ...
                    if not current_price:
                        print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Skipping card {idx+1}: No price found")
                        continue
                    ...
                    if not sqft:
                        print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Skipping card {idx+1}: No square footage found")
                        continue
                    ...
                    print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Card {idx+1}: {plan_data}")
                    listings.append(plan_data)
                    
                except Exception as e:
                    print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Error processing card {idx+1}: {e}")
                    continue
            
            print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Successfully processed {len(listings)} items")
            return listings
            
        except Exception as e:
            print(f"[DavidWeekleyHomesMyrtleCreekPlanScraper] Error: {e}")
            return []
