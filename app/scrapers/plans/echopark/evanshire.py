...
class EvanshireEchoParkPlanScraper(BaseScraper):
    ...
    def extract_plan_data(self, plan_card, community_name, city):
        ...
        try:
            ...
            print(f"[EvanshireEchoParkPlanScraper] Error extracting plan data: {e}")
            return None

    def fetch_plans(self) -> List[Dict]:
        try:
            print(f"[EvanshireEchoParkPlanScraper] Fetching URL: {self.URL}")
            ...
            print(f"[EvanshireEchoParkPlanScraper] Response status: {resp.status_code}")
            
            if resp.status_code != 200:
                print(f"[EvanshireEchoParkPlanScraper] Request failed with status {resp.status_code}")
                return []
            ...
            print(f"[EvanshireEchoParkPlanScraper] Found {len(swiper_slides)} swiper slides")
            
            for idx, slide in enumerate(swiper_slides):
                try:
                    print(f"[EvanshireEchoParkPlanScraper] Processing slide {idx+1}")
                    
                    # Find plan card within the slide
                    plan_card = slide.find('div', class_='plan-card')
                    if not plan_card:
                        print(f"[EvanshireEchoParkPlanScraper] Slide {idx+1}: Not a plan-card, skipping")
                        continue
                    
                    print(f"[EvanshireEchoParkPlanScraper] Slide {idx+1}: Processing plan card")
                    
                    # Extract plan data
                    plan_data = self.extract_plan_data(plan_card, "Echo Park", "Duluth")
                    
                    if not plan_data:
                        print(f"[EvanshireEchoParkPlanScraper] Slide {idx+1}: Failed to extract plan data")
                        continue
                    
                    # Check for duplicate plan names
                    if plan_data['plan_name'] in seen_plan_names:
                        print(f"[EvanshireEchoParkPlanScraper] Slide {idx+1}: Duplicate plan name '{plan_data['plan_name']}', skipping")
                        continue
                    ...
                    if not plan_data.get('plan_name') or not plan_data.get('price'):
                        print(f"[EvanshireEchoParkPlanScraper] Slide {idx+1}: Missing required fields")
                        continue
                    
                    print(f"[EvanshireEchoParkPlanScraper] Slide {idx+1}: {plan_data}")
                    listings.append(plan_data)
                    
                except Exception as e:
                    print(f"[EvanshireEchoParkPlanScraper] Error processing slide {idx+1}: {e}")
                    continue
            
            print(f"[EvanshireEchoParkPlanScraper] Successfully processed {len(listings)} listings")
            return listings
            
        except Exception as e:
            print(f"[EvanshireEchoParkPlanScraper] Error: {e}")
            return []
