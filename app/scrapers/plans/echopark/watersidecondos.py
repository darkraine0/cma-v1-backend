...
class WatersideCondosEchoParkPlanScraper(BaseScraper):
    ...
    def extract_plan_data(self, plan_card, community_name, city):
        ...
        try:
            ...
            print(f"[WatersideCondosEchoParkPlanScraper] Error extracting plan data: {e}")
            return None

    def fetch_plans(self) -> List[Dict]:
        try:
            print(f"[WatersideCondosEchoParkPlanScraper] Fetching URL: {self.URL}")
            ...
            print(f"[WatersideCondosEchoParkPlanScraper] Response status: {resp.status_code}")
            
            if resp.status_code != 200:
                print(f"[WatersideCondosEchoParkPlanScraper] Request failed with status {resp.status_code}")
                return []
            ...
            print(f"[WatersideCondosEchoParkPlanScraper] Found {len(swiper_slides)} swiper slides")
            
            for idx, slide in enumerate(swiper_slides):
                try:
                    print(f"[WatersideCondosEchoParkPlanScraper] Processing slide {idx+1}")
                    
                    plan_card = slide.find('div', class_='plan-card')
                    if not plan_card:
                        print(f"[WatersideCondosEchoParkPlanScraper] Slide {idx+1}: Not a plan-card, skipping")
                        continue
                    
                    print(f"[WatersideCondosEchoParkPlanScraper] Slide {idx+1}: Processing plan card")
                    
                    plan_data = self.extract_plan_data(plan_card, "Echo Park", "Peachtree Corners")
                    
                    if not plan_data:
                        print(f"[WatersideCondosEchoParkPlanScraper] Slide {idx+1}: Failed to extract plan data")
                        continue
                    
                    if plan_data['plan_name'] in seen_plan_names:
                        print(f"[WatersideCondosEchoParkPlanScraper] Slide {idx+1}: Duplicate plan name '{plan_data['plan_name']}', skipping")
                        continue
                    
                    seen_plan_names.add(plan_data['plan_name'])
                    
                    if not plan_data.get('plan_name') or not plan_data.get('price'):
                        print(f"[WatersideCondosEchoParkPlanScraper] Slide {idx+1}: Missing required fields")
                        continue
                    
                    print(f"[WatersideCondosEchoParkPlanScraper] Slide {idx+1}: {plan_data}")
                    listings.append(plan_data)
                    
                except Exception as e:
                    print(f"[WatersideCondosEchoParkPlanScraper] Error processing slide {idx+1}: {e}")
                    continue
            
            print(f"[WatersideCondosEchoParkPlanScraper] Successfully processed {len(listings)} listings")
            return listings
            
        except Exception as e:
            print(f"[WatersideCondosEchoParkPlanScraper] Error: {e}")
            return []
