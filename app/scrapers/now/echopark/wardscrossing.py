...
class WardsCrossingEchoParkNowScraper(BaseScraper):
    URL = "https://theprovidencegroup.com/new-homes/ga/johns-creek/wards-crossing-townhomes/13811/"
    ...
    def extract_property_data(self, property_card, community_name, city):
        ...
        try:
            ...
            plan_data = {
                ...
                "plan_name": plan_name,
                "company": "Wards Crossing Townhomes",
                "community": "Echo Park",
                "type": "now",
                "beds": beds,
                "baths": baths,
            ...
            print(f"[WardsCrossingEchoParkNowScraper] Error extracting property data: {e}")
            return None

    def fetch_plans(self) -> List[Dict]:
        try:
            print(f"[WardsCrossingEchoParkNowScraper] Fetching URL: {self.URL}")
            ...
            print(f"[WardsCrossingEchoParkNowScraper] Response status: {resp.status_code}")
            
            if resp.status_code != 200:
                print(f"[WardsCrossingEchoParkNowScraper] Request failed with status {resp.status_code}")
                return []
            ...
            print(f"[WardsCrossingEchoParkNowScraper] Found {len(swiper_slides)} swiper slides")
            
            for idx, slide in enumerate(swiper_slides):
                try:
                    print(f"[WardsCrossingEchoParkNowScraper] Processing slide {idx+1}")
                    ...
                    if not property_card or 'spec-map-item' not in property_card.get('class', []):
                        print(f"[WardsCrossingEchoParkNowScraper] Slide {idx+1}: Not a spec-map-item, skipping")
                        continue
                    
                    print(f"[WardsCrossingEchoParkNowScraper] Slide {idx+1}: Processing property card")
                    
                    # Extract property data
                    plan_data = self.extract_property_data(property_card, "Echo Park", "Johns Creek")
                    
                    if not plan_data:
                        print(f"[WardsCrossingEchoParkNowScraper] Slide {idx+1}: Failed to extract property data")
                        continue
                    
                    # Check for duplicate addresses
                    if plan_data['address'] in seen_addresses:
                        print(f"[WardsCrossingEchoParkNowScraper] Slide {idx+1}: Duplicate address '{plan_data['address']}', skipping")
                        continue
                    ...
                    if not plan_data.get('plan_name') or not plan_data.get('price'):
                        print(f"[WardsCrossingEchoParkNowScraper] Slide {idx+1}: Missing required fields")
                        continue
                    
                    print(f"[WardsCrossingEchoParkNowScraper] Slide {idx+1}: {plan_data}")
                    listings.append(plan_data)
                    
                except Exception as e:
                    print(f"[WardsCrossingEchoParkNowScraper] Error processing slide {idx+1}: {e}")
                    continue
            
            print(f"[WardsCrossingEchoParkNowScraper] Successfully processed {len(listings)} listings")
            return listings
            
        except Exception as e:
            print(f"[WardsCrossingEchoParkNowScraper] Error: {e}")
            return []
