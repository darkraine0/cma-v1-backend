...
class WatersideTownhomesEchoParkNowScraper(BaseScraper):
    ...
    def extract_property_data(self, property_card, community_name, city):
        ...
        try:
            ...
            print(f"[WatersideTownhomesEchoParkNowScraper] Error extracting property data: {e}")
            return None

    def fetch_plans(self) -> List[Dict]:
        try:
            print(f"[WatersideTownhomesEchoParkNowScraper] Fetching URL: {self.URL}")
            ...
            print(f"[WatersideTownhomesEchoParkNowScraper] Response status: {resp.status_code}")
            
            if resp.status_code != 200:
                print(f"[WatersideTownhomesEchoParkNowScraper] Request failed with status {resp.status_code}")
                return []
            ...
            print(f"[WatersideTownhomesEchoParkNowScraper] Found {len(swiper_slides)} swiper slides")
            
            for idx, slide in enumerate(swiper_slides):
                try:
                    print(f"[WatersideTownhomesEchoParkNowScraper] Processing slide {idx+1}")
                    ...
                    if not property_card or 'spec-map-item' not in property_card.get('class', []):
                        print(f"[WatersideTownhomesEchoParkNowScraper] Slide {idx+1}: Not a spec-map-item, skipping")
                        continue
                    
                    print(f"[WatersideTownhomesEchoParkNowScraper] Slide {idx+1}: Processing property card")
                    
                    # Extract property data
                    plan_data = self.extract_property_data(property_card, "Echo Park", "Peachtree Corners")
                    
                    if not plan_data:
                        print(f"[WatersideTownhomesEchoParkNowScraper] Slide {idx+1}: Failed to extract property data")
                        continue
                    
                    # Check for duplicate addresses
                    if plan_data['address'] in seen_addresses:
                        print(f"[WatersideTownhomesEchoParkNowScraper] Slide {idx+1}: Duplicate address '{plan_data['address']}', skipping")
                        continue
                    ...
                    if not plan_data.get('plan_name') or not plan_data.get('price'):
                        print(f"[WatersideTownhomesEchoParkNowScraper] Slide {idx+1}: Missing required fields")
                        continue
                    
                    print(f"[WatersideTownhomesEchoParkNowScraper] Slide {idx+1}: {plan_data}")
                    listings.append(plan_data)
                    
                except Exception as e:
                    print(f"[WatersideTownhomesEchoParkNowScraper] Error processing slide {idx+1}: {e}")
                    continue
            
            print(f"[WatersideTownhomesEchoParkNowScraper] Successfully processed {len(listings)} listings")
            return listings
            
        except Exception as e:
            print(f"[WatersideTownhomesEchoParkNowScraper] Error: {e}")
            return []
