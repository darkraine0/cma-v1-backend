...
class EvanshireEchoParkNowScraper(BaseScraper):
    URL = "https://theprovidencegroup.com/new-homes/ga/duluth/evanshire-townhomes/13813/"
    ...
    def extract_property_data(self, property_card, community_name, city):
        ...
        try:
            ...
            print(f"[EvanshireEchoParkNowScraper] Error extracting property data: {e}")
            return None

    def fetch_plans(self) -> List[Dict]:
        try:
            print(f"[EvanshireEchoParkNowScraper] Fetching URL: {self.URL}")
            ...
            print(f"[EvanshireEchoParkNowScraper] Response status: {resp.status_code}")
            
            if resp.status_code != 200:
                print(f"[EvanshireEchoParkNowScraper] Request failed with status {resp.status_code}")
                return []
            ...
            print(f"[EvanshireEchoParkNowScraper] Found {len(swiper_slides)} swiper slides")
            
            for idx, slide in enumerate(swiper_slides):
                try:
                    print(f"[EvanshireEchoParkNowScraper] Processing slide {idx+1}")
                    ...
                    if not property_card or 'spec-map-item' not in property_card.get('class', []):
                        print(f"[EvanshireEchoParkNowScraper] Slide {idx+1}: Not a spec-map-item, skipping")
                        continue
                    
                    print(f"[EvanshireEchoParkNowScraper] Slide {idx+1}: Processing property card")
                    
                    # Extract property data
                    plan_data = self.extract_property_data(property_card, "Echo Park", "Duluth")
                    
                    if not plan_data:
                        print(f"[EvanshireEchoParkNowScraper] Slide {idx+1}: Failed to extract property data")
                        continue
                    
                    # Check for duplicate addresses
                    if plan_data['address'] in seen_addresses:
                        print(f"[EvanshireEchoParkNowScraper] Slide {idx+1}: Duplicate address '{plan_data['address']}', skipping")
                        continue
                    ...
                    if not plan_data.get('plan_name') or not plan_data.get('price'):
                        print(f"[EvanshireEchoParkNowScraper] Slide {idx+1}: Missing required fields")
                        continue
                    
                    print(f"[EvanshireEchoParkNowScraper] Slide {idx+1}: {plan_data}")
                    listings.append(plan_data)
                    
                except Exception as e:
                    print(f"[EvanshireEchoParkNowScraper] Error processing slide {idx+1}: {e}")
                    continue
            
            print(f"[EvanshireEchoParkNowScraper] Successfully processed {len(listings)} listings")
            return listings
            
        except Exception as e:
            print(f"[EvanshireEchoParkNowScraper] Error: {e}")
            return []
